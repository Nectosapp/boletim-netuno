# -*- coding: utf-8 -*-
r"""
Netuno — Boletim de Mercado (modelo solicitado) — v3
Com BCC por padrão, delay antes do envio e logging em arquivo.
...
Uso típico:
  py netuno_boletim_modelo.py --preview
  py netuno_boletim_modelo.py --send --delay 25 --from "gustavo.hamer@r2fcapital.com.br"
  py netuno_boletim_modelo.py --send --no-bcc
"""
    ...
    for i in range(0, len(symbols), chunk):
        ...
        for h in hosts:
            try:
                ...
                logging.warning("Yahoo v7 %s falhou %s: %s", h, syms, ex)
                time.sleep(0.6)
        if not ok:
            logging.warning("v7 falhou de vez %s", syms)
        time.sleep(0.25)
    ...
    for sym in symbols:
        ...
        for h in hosts:
            try:
                ...
                logging.warning("Yahoo v8 %s falhou %s: %s", h, sym, ex)
                time.sleep(0.4)
    return out
...
def parse_rss(url: str) -> List[Dict]:
    ...
    try:
        ...
        logging.warning("RSS fail %s: %s", url, ex)
    return items
...
def parse_html_list(url: str, css_sel: str, attr: str) -> List[str]:
    try:
        ...
        logging.warning("HTML list fail %s: %s", url, ex)
        return []
...
def extract_article_text(url: str) -> str:
    try:
        ...
        try:
            ...
            logging.warning("extract fail %s: %s", url, ex)
            return ""
        ...
        for cfg in src.get("html", []):
            ...
            for lk in links[:MAX_PER_FONTE]:
                try:
                    ...
                    logging.warning("fetch link fail %s: %s", lk, ex)
            time.sleep(SLEEP)
...
def mk_paragrafo_internacional(items: List[Dict], max_chars=700) -> str:
    ...
    if len(par) > max_chars: par = par[:max_chars].rsplit(" ",1)[0]+"…"
...
def _try_set_sender_account(mail, outlook_session, from_hint: Optional[str]) -> None:
    """Seleciona a conta remetente quando houver múltiplas contas (e-mail ou parte do display name)."""
    if not from_hint: return
    try:
        for i in range(1, outlook_session.Accounts.Count + 1):
            ...
            if from_hint.lower() in name:
                ...
                logging.info("Remetente selecionado: %s", mail.SentOnBehalfOfName)
                return
        logging.warning("Remetente '%s' não encontrado; usando padrão.", from_hint)
    except Exception as ex:
        logging.warning("Falha ao selecionar conta remetente: %s", ex)
...
def enviar_email_outlook(
    ...
    html_corpo: str,
    MODE: str = "SEND",
    from_account_hint: Optional[str] = None,
    ...
    try:
        ...
        if to_addr:
            r = recips.Add(to_addr); r.Type = 1  # To
        for addr in cc_addrs or []:
            r = recips.Add(addr); r.Type = 2     # CC
        for addr in bcc_addrs or []:
            r = recips.Add(addr); r.Type = 3     # BCC

        if not recips.ResolveAll():
            raise RuntimeError("Falha ao resolver um ou mais destinatários.")

        if MODE.upper() == "DISPLAY_ONLY":
            mail.Display()
            logging.info("E-mail aberto (DISPLAY_ONLY).")
        elif MODE.upper() == "SAVE_DRAFT":
            mail.Save()
            logging.info("E-mail salvo (SAVE_DRAFT).")
        else:
            # delay para reduzir risco de bloqueio
            if delay_seconds and delay_seconds > 0:
                logging.info("Aguardando %ss antes de enviar...", delay_seconds)
                time.sleep(delay_seconds)
            mail.Send()
            logging.info("E-mail enviado (SEND).")

    except Exception as e:
        logging.error("Falha no envio via Outlook: %s", e)
        logging.error("Dicas: Outlook aberto/logado; mesma elevação do Python; Acesso Programático liberado.")
        raise
    finally:
        try:
...
# ================= MAIN =================
def run(mode: str = "SEND", from_hint: Optional[str] = None, use_bcc: bool = True, delay_seconds: int = 20):
    ...
    rodape = (
        ...
        "Fontes: InfoMoney, Mais Retorno, Yubb, Bora Investir/B3, "
    ...
    # To/BCC strategy
    if use_bcc:
        to_addr = DESTINATARIOS[1] if len(DESTINATARIOS) > 1 else DESTINATARIOS[0]
        # TO = preferencialmente corporativo (ajuste se quiser outro)
        if not re.search(r"@r2fcapital\.com\.br$", to_addr, flags=re.I):
            to_addr = DESTINATARIOS[0]
        bcc_list = [d for d in DESTINATARIOS if d.lower() != to_addr.lower()]
        cc_list = []
    else:
        # todos em To
        to_addr = DESTINATARIOS[0]
    ...
    logging.info("Preparando envio. TO=%s | CC=%s | BCC=%s | mode=%s | delay=%ss",
                 to_addr, cc_list, bcc_list, mode, delay_seconds)
...
if __name__ == "__main__":
    try:
        ...
        ap.add_argument("--no-bcc", action="store_true", help="coloca todos em To/CC (desativa BCC)")
        ap.add_argument("--delay", type=int, default=20, help="segundos para aguardar antes de enviar (padrão: 20)")
        ap.add_argument("--log-path", default="netuno_boletim.log", help="arquivo de log (padrão: netuno_boletim.log)")
        args = ap.parse_args()
