# -*- coding: utf-8 -*-
r"""
Netuno — Boletim de Mercado (modelo solicitado) — v2 com Fallback de Cotações
Fontes:
...
def _blocked_text(txt: str) -> bool:
    """Filtra textos de paywall/deny para não poluir o parágrafo."""
...
def mk_paragrafo_internacional(items: List[Dict], max_chars=700) -> str:
    # junta 3–5 mini-resumos que não estejam bloqueados
    parts = []
    for it in items[:6]:
...
def bloco_internacional(items: List[Dict]) -> str:
    ...
    # cotações globais reutilizando bloco_cotacoes (vamos pegar só o bloco global)
    cot = bloco_cotacoes()
    # extrai o segundo <h3> (globais) + seu <p>
    parts = cot.split("<h3")
    glob = ""
    if len(parts) >= 3:
        glob = "<h3" + parts[2]  # inclui título e <p> de globais
    return (
...
def main():
    ...
    rodape = (
        ...
        "Fontes: Valor Investe, Mais Retorno, Yubb, Bora Investir/B3, "
