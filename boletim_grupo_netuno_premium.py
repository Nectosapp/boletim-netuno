# -*- coding: utf-8 -*-
r"""
Netuno — Boletim de Mercado — Premium Edition v1.0
Design sofisticado para clientes exigentes do mercado financeiro.
- Layout moderno com cards e gradientes
- Tipografia profissional
- Paleta de cores premium (azul escuro, dourado, tons neutros)
...
COLORS = {
    "primary": "#00acad",      # Teal R2F Capital (cor principal da marca)
    "primary_dark": "#008b8c", # Teal escuro para gradientes
    "secondary": "#1a2744",    # Azul escuro para contraste
    "accent": "#00acad",       # Teal como destaque
    "accent_hover": "#61a229", # Verde para hover
    "positive": "#28a745",     # Verde R2F para alta
    "negative": "#dc3545",     # Vermelho para baixa
    "neutral": "#6c757d",      # Cinza neutro
    "text_dark": "#000000",    # Texto principal (preto)
    "text_medium": "#333333",  # Texto secundario
    "text_light": "#6c757d",   # Texto terciario
    "bg_light": "#f8f9fa",     # Fundo claro
    "bg_card": "#ffffff",      # Fundo dos cards
    "border": "#e3e1e8",       # Bordas R2F
    "divider": "#dee2e6",      # Divisores
    "header_bg": "#1a2744",    # Fundo do header (azul escuro)
}
...
def get_email_styles() -> str:
    """Retorna os estilos CSS inline para máxima compatibilidade com clientes de email."""
    return f"""
    <style type="text/css">
...
def get_header_html() -> str:
    """Cabecalho com identidade visual R2F Capital / Grupo Netuno."""
...
def get_footer_html() -> str:
    """Rodape profissional estilo R2F Capital."""
    return f"""
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color: {COLORS['header_bg']};">
        <tr>
...
def bloco_cotacoes_premium() -> str:
    """Cotacoes em formato de tabela limpa estilo Seeking Alpha."""
...
def render_news_card(it: Dict, traduzir: bool = False) -> str:
    """Renderiza uma noticia como card estilo Brazil Journal / XP."""
...
def render_section_header(title: str, subtitle: str = "") -> str:
    """Renderiza o cabecalho de uma secao estilo Brazil Journal."""
    return f"""
    <tr>
...
def bloco_destaques_premium(news: Dict[str, List[Dict]], total: int = 4) -> str:
    """Destaques em formato clean estilo Brazil Journal."""
    pool = []
...
def bloco_noticias_premium(items: List[Dict], title: str, traduzir: bool = False, limit: int = 5, subtitle: str = "") -> str:
    """Bloco de noticias em formato limpo estilo Brazil Journal."""
    cards = []
...
def enviar_email_outlook(dest: List[str], assunto: str, html_corpo: str):
    ...
    # Força envio pela conta R2F Capital específica
    conta_encontrada = False
    try:
        for acc in outlook.Session.Accounts:
            # Verifica tanto DisplayName quanto SmtpAddress
            email_acc = acc.SmtpAddress.lower() if hasattr(acc, 'SmtpAddress') else ""
            display_acc = acc.DisplayName.lower() if hasattr(acc, 'DisplayName') else ""

            if CONTA_REMETENTE.lower() in email_acc or CONTA_REMETENTE.lower() in display_acc:
                mail._oleobj_.Invoke(*(64209, 0, 8, 0, acc))
                print(f"[OK] Envio via conta: {acc.SmtpAddress}")
                conta_encontrada = True
                break
            elif "r2fcapital.com.br" in email_acc or "r2fcapital.com.br" in display_acc:
                mail._oleobj_.Invoke(*(64209, 0, 8, 0, acc))
                print(f"[!] Conta especifica nao encontrada, usando: {acc.SmtpAddress}")
                conta_encontrada = True
                break

        if not conta_encontrada:
            print(f"[!] Conta {CONTA_REMETENTE} nao encontrada no Outlook.")
            print("    Contas disponiveis:")
            for acc in outlook.Session.Accounts:
                print(f"    - {acc.SmtpAddress}")
            print("    Enviando pela conta padrao...")

    except Exception as e:
        print(f"[!] Falha ao definir conta especifica: {e}")

    mail.To = ";".join(dest)
    mail.Subject = assunto
    mail.HTMLBody = html_corpo
    mail.Send()
    print("[OK] E-mail enviado com sucesso via Outlook.")
